...
namespace EazyTools.SoundManager
{
    public class SoundManager : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            // Update music
            foreach (int key in musicAudio.Keys)
            {
               musicAudio[key].Update();
            }

            // Update sound fx
            
            foreach (int key in soundsAudio.Keys)
            {
               soundsAudio[key].Update();
            }
        ...
        void Init()
        {
            if (!initialized)
            {
                ...
                ignoreDuplicateSounds = true;
                ignoreDuplicateUISounds = true;
        ...
        public static int PlayMusic(AudioClip clip, float volume, bool loop, bool persist, float fadeInSeconds, float fadeOutSeconds, float currentMusicfadeOutSeconds, Transform sourceTransform)
        {
            ...
            if(ignoreDuplicateMusic)
            {
                ...
                foreach (int key in keys)
                {
                    if (musicAudio[key].audioSource != null && musicAudio[key].audioSource.clip == clip)
                    {
        ...
        public static int PlaySound(AudioClip clip, float volume, bool loop, Transform sourceTransform)
        {
            ...
            if (ignoreDuplicateSounds)
            {
                ...
                foreach (int key in keys)
                {
                    if (soundsAudio[key].audioSource != null && soundsAudio[key].audioSource.clip == clip)
                    {
        ...
        {
            if (musicAudio != null && musicAudio.Keys != null)
            {
                List<int> keys = new List<int>(musicAudio.Keys);
                foreach (int key in keys)
                {
        ...
        public static void PauseAllFrontMusic()
        {
            if (musicAudio != null && musicAudio.Keys != null)
            {
                List<int> keys = new List<int>(musicAudio.Keys);
